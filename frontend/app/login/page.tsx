...
export default function LoginPage() {
  ...
  async function handleSubmit(event: FormEvent<HTMLFormElement>) {
    ...
    setLoading(true);
    try {
      const result = await authenticate(email, password, "login");
    ...
    <main className="flex min-h-screen items-center justify-center px-6 py-12">
      <section className="w-full max-w-md rounded-3xl border border-slate-800 bg-slate-900/80 p-8 shadow-2xl shadow-slate-950/50">
        ...
        <form className="mt-8 space-y-4" onSubmit={handleSubmit}>
          ...
          <button disabled={loading} type="submit" className="w-full rounded-xl bg-cyan-400 px-4 py-3 font-semibold text-slate-950 hover:bg-cyan-300 disabled:cursor-not-allowed disabled:opacity-60">{loading ? "登录中…" : "登录"}</button>
        </form>
        <p className="mt-6 text-center text-sm text-slate-400">还没有账户？ <Link href="/register" className="text-cyan-300 hover:text-cyan-200">立即注册</Link></p>
      </section>
    </main>
  );
}